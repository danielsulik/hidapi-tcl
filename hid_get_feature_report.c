_wrap_hid_get_feature_report(ClientData clientData SWIGUNUSED, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (SWIG_GetArgs(interp, objc, objv,"oo:hid_get_feature_report device count ",(void *)0,(void *)0) == TCL_ERROR) SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1,SWIGTYPE_p_hid_device, 0 |  0 );
  if (!SWIG_IsOK(res1)) {
    SWIG_exception_fail(SWIG_ArgError(res1), "in method '" "hid_get_feature_report" "', argument " "1"" of type '" "hid_device *""'");
  }
  arg1 = (hid_device *)(argp1);
  ecode3 = SWIG_AsVal_unsigned_SS_int SWIG_TCL_CALL_ARGS_2(objv[2], &val3);
  if (!SWIG_IsOK(ecode3)) {
    SWIG_exception_fail(SWIG_ArgError(ecode3), "in method '" "hid_get_feature_report" "', argument " "3"" of type '" "unsigned int""'");
  }
  ...
  result = (int)hid_get_feature_report(arg1,rxBuf,arg3);
